...
export default function Contact() {
  ...
  return (
    <footer id="contact" className="bg-uniform-navy pt-32 pb-12 px-6 overflow-hidden relative" style={{ color: '#0f1a2b' }}>
      {/* Decorative logo background deco */}
      <div className="absolute top-20 right-[-10%] w-[35vw] h-[35vw] opacity-[0.03] pointer-events-none select-none">
        <img src="input_file_3.png" alt="" className="w-full h-full object-contain grayscale" referrerPolicy="no-referrer" />
      </div>
        ...
        <div className="grid lg:grid-cols-2 gap-24 items-start mb-32">
          <div>
            ...
            <h3 className="text-6xl font-serif font-black text-white leading-[0.9] mb-12 max-w-md">
              {lang === 'EN' ? (
                <>The Pride of <br /><span className="text-uniform-tan italic underline decoration-uniform-red decoration-2 underline-offset-8">Limbe</span>.</>
              ) : (
                <>La Fierté de <br /><span className="text-uniform-tan italic underline decoration-uniform-red decoration-2 underline-offset-8">Limbe</span>.</>
              )}
            </h3>

            <div className="space-y-10 relative z-10">
               <div className="flex gap-6 group cursor-pointer">
                  <div className="w-14 h-14 bg-white/5 rounded-xl border border-white/10 flex items-center justify-center shrink-0 group-hover:bg-uniform-red group-hover:border-uniform-red transition-all">
                     <MapPin className="w-6 h-6 text-uniform-red" />
                  </div>
                  <div>
                     <p className="text-xs font-black text-uniform-red uppercase tracking-widest mb-1">{t.contact.location_label}</p>
               ...
               <div className="flex gap-6 group cursor-pointer">
                  <div className="w-14 h-14 bg-white/5 rounded-xl border border-white/10 flex items-center justify-center shrink-0 group-hover:bg-uniform-tan group-hover:border-uniform-tan transition-all">
                     <Phone className="w-6 h-6 text-uniform-tan" />
                  </div>
                  <div>
                     <p className="text-xs font-black text-uniform-tan uppercase tracking-widest mb-1">{t.contact.phone_label}</p>
        ...
        <div className="pt-12 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-8">
           ...
           <div className="flex gap-4">
              {[Facebook, Twitter, Instagram].map((Icon, idx) => (
                 <a key={idx} href="#" className="w-10 h-10 bg-white/5 rounded-full flex items-center justify-center text-slate-400 hover:bg-uniform-red hover:text-white transition-all">
